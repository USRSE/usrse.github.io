## 🗞️ Community News 🗞️

<a name="news"></a>

<!--Someone you want to shoutout? DO IT HERE!-->

### New newsletter alert: User & Developer Experience

Our very own Hannah Coon and the UX working group will be sharing the latest and greatest developments in User and Developer Experience — UDX!

UDX in this case includes anything oriented toward making developers' or users' interactions with tech easier (be that via features, docs, policies, etc.). The ux-wg will also be highlighting relevant events, including talks/workshops/BoFs/posters at USRSE26. If you want your presentation included in the list, reach out to Hannah with the title and a very brief description of how it relates to UX or UDX work.
And, if you've got other relevant events or content you'd like promoted (work of yours or someone else's), send that along too!

### **Community Shoutouts**

Check out this photo of our friends across the pond at RSECon 2026! You may recognize a familiar face or two...

<p align="center">
  <img src="{{ site.baseurl }}/assets/img/newsletter-202609/RSEcon26_Thurs_Delegates_003.JPG" alt="Image of RSECon 2026 attendees" style="width: 750px; height: auto;">
</p>

> Did you know that we have a community Code of Conduct? Anyone is able to view it in the
`#code_of_conduct` Slack channel, under `Files`!

## Community Spotlight

🌱 Our community is full of people doing fascinating research and software work,  and we want to put a face to it. Starting this month, we'll be featuring a group of different members in a regular spotlight: what they work on, a tool they can't live without, and how they found their way into RSE work.

We'd love to feature YOU. It takes about 5 minutes to fill out, and nothing gets posted without your okay: [https://forms.gle/dXqVsHKiHnot2u449](https://forms.gle/dXqVsHKiHnot2u449)

Email [Pengyin Shan](mailto:pengyins@illinois.edu) for any questions!


### **Community Calls**

<!--Community call news goes here-->
<!-- Add a link to the recording of the call if available 
**TBD last call**
<div style="position: relative; width: 100%; max-width: 640px; margin: 0 auto; overflow: hidden; background: #000; aspect-ratio: 16 / 9;">
  <img src="" 
       alt="YouTube Video Thumbnail" 
       style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; object-fit: cover; cursor: pointer;" 
       onclick="this.outerHTML='<iframe width=\'640\' height=\'360\' src=\'\' title=\'YouTube video player\' frameborder=\'0\' allow=\'accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share\' allowfullscreen style=\'position: absolute; top: 0; left: 0; width: 100%; height: 100%;\'></iframe>'">
</div>
-->

Check out our YouTube channel for recordings of past calls: [https://www.youtube.com/@us_rse](https://www.youtube.com/@us_rse).
