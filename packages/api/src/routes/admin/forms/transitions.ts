...
export const adminFormTransitionsRoute = new Hono<AppEnv>();
...
adminFormTransitionsRoute.post(
  "/",
  ...
  async (c) => {
    ...
    const existing = await db
      .select({ status: forms.status, authorId: forms.authorId })
      .from(forms)
