...
### 2026-05-19: Group rename is a frontend gate, not an API restriction (Approved)

- **Decision:** Group `name` is editable through the existing `PATCH /api/admin/groups/:id` endpoint (1–120 chars, audited via the standard `groups.update` action) and is gated to `super_admin` purely in the admin UI. Chairs still cannot rename. `slug` and `type` remain locked because the PATCH schema does not accept them — the slug is referenced in `<title>` and external links, and `type` drives public routing.
- **Why:** The API already had `name` in the patch schema and `canEditGroup` already granted super_admins access, but the admin `GroupDetailPage` hardcoded `readOnly` on the input with a hint ("Contact a super_admin to rename") that promised a path which didn't exist. Keeping the rename gate in the UI (rather than splitting a new `canRenameGroup` policy) preserves the chair flow (chairs touch description, charter, slack, links, roster — never the name) without introducing a parallel policy.
- **Result:** `apps/admin/src/pages/groups/GroupDetailPage.tsx` reads `actor.systemTier >= 1` and renders Name as a controlled input wired into `draftName` / `saveIdentity` when staff; chairs see the same read-only input with the original hint. Single-file fix in commit `09c0ca8`.

### 2026-05-19: Slack username linking via two-axis fuzzy scoring (Approved)

- **Decision:** A standalone TS script (`packages/api/scripts/link-slack-users.ts`) reads `.data/us-rse-groups.csv` (the Slack-exported channel roster), scores each unique Slack username against active US-RSE profiles on two axes — email-local-to-username similarity (max 100 points) and normalized-display-name similarity (max 90) — and writes `profiles.slack_username` for high (≥100) and medium (≥50) confidence matches. Low and none decisions dump to a review CSV. After the username pass, the script walks the channel roster a second time and inserts `group_memberships` rows for each `(channel, matched user)` pair. Dry-run by default; `--commit` writes; `--memberships-only` skips the username pass.
- **Why:** Slack rosters live in CSV form (the workspace export); US-RSE profiles live in Postgres; matching is fuzzy because Slack usernames are often email-locals, sometimes display-name slugs, and occasionally neither. A pure-SQL `LOWER(email) LIKE '%slack_username%'` was tested early and produced too many false positives on common name fragments. Splitting into a scoring helper (`scripts/lib/slackMatch.ts`, DB-free, unit-tested with fixtures) plus a thin driver lets us run the matcher inside `vitest` and fine-tune thresholds without DB roundtrips. Demoting ambiguous winners by one confidence tier (when the runner-up is close) is the safety valve that pushes borderline calls into the review CSV for human resolution.
- **Result:** ~200-line script + ~127-line scoring lib + ~194-line test suite, plus a `link:slack-users` npm script entrypoint. Admin `MemberDetailPage` now surfaces a `slack_username` input so the human-review CSV decisions can be applied without re-running the matcher. Merged via PR #1987 (`416b032`).

### 2026-05-16: Public organizations directory uses an optional-actor visibility predicate (Approved)

- **Decision:** Public organization data lives at `GET /api/organizations` (list, paginated + filterable by type/country) and `GET /api/organizations/:id` (profile + visibility-filtered roster). Both routes use a new `optionalActor` middleware that extracts and validates the WorkOS token *if present* but never rejects unauthenticated requests. The visibility predicate (`lib/orgVisibility.ts`) returns the full roster when the viewer is staff or an org member; otherwise only members whose profile satisfies `isPublic && isDiscoverable` appear. The legacy `/resources/directory` page is retired in favor of `/orgs` + `/orgs/:id`, with backfill scripts (`backfill-org-types`, `backfill-org-locations`) seeding type classifications and Photon/Wikidata-derived coordinates.
- **Why:** A binary `requireAuth` middleware can't model "public by default, richer when authenticated" without duplicating routes. Pulling auth extraction into a separate `optionalActor` keeps the route handlers single-pathed: they ask the predicate for the visible roster regardless of who's calling. The predicate's three booleans (`isPublic`, `isDiscoverable`, plus actor membership) cleanly express the three viewer classes — public visitor, signed-in member, staff/colleague — without bolting role checks onto each query. Backfill via Wikidata + Wikipedia + Photon (rather than a manual seed) keeps the directory honest as orgs are added through admin: the script can be re-run idempotently to fill blanks.
- **Result:** `packages/api/src/routes/organizations.ts` (379 lines) + `packages/api/src/middleware/optionalActor.ts` (123 lines) + `packages/api/src/lib/orgVisibility.ts` (70 lines) + 4 backfill/seed scripts, 5 new admin/web React pages under `/orgs`, and migration `0020_organizations_public_profile.sql` (adds type, country, description, audit columns). Merged via PR #1984 (`50a8078`). Spec: [`2026-05-16-org-directory-design.md`](../superpowers/specs/2026-05-16-org-directory-design.md). Plan: [`2026-05-16-org-directory-implementation.md`](../superpowers/plans/2026-05-16-org-directory-implementation.md).
