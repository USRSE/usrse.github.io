...
const eventsPages = [
  { path: "/events", label: "Upcoming Events", shortLabel: "Events" },
  { path: "/events/calendar", label: "Calendar", shortLabel: "Cal" },
  { path: "/events/usrse26", label: "USRSE'26 Conference", shortLabel: "USRSE26" },
];
...
export function EventsLayout({
  ...
  return (
    <div className="min-h-screen bg-white">
      {/* Hero — warm dark gradient, distinct from About (purple) and Community (teal) */}
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-10">
          <nav aria-label="Breadcrumb" className="mb-6 animate-fade-in">
            <ol className="flex items-center gap-2 text-sm">
              ...
              <li><span className="text-white/50">Events</span></li>
          ...
          <div className="flex items-center gap-2 mt-8 animate-slide-up flex-wrap" style={{ animationDelay: "300ms" }}>
            {eventsPages.map((page) => {
              ...
              return (
                ...
                >
                  <span className={`w-1.5 h-1.5 rounded-full ${isCurrent ? "bg-teal-400" : "bg-white/30"}`} />
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 py-16 lg:py-20">
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-16">
          ...
          <aside className="hidden lg:block w-48 shrink-0">
            <nav className="sticky top-24" aria-label="Events pages">
              <ul className="space-y-0">
                {eventsPages.map((page, i) => {
                  const isCurrent = location.pathname === page.path;
                  return (
                    <li key={page.path}>
                      <Link
