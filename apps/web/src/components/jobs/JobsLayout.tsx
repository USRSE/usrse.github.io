...
const jobsPages = [
  { path: "/jobs", label: "Job Board", shortLabel: "Jobs" },
  { path: "/jobs/submit", label: "Post a Job", shortLabel: "Post" },
  { path: "/jobs/volunteer", label: "Volunteer with US-RSE", shortLabel: "Volunteer" },
];
...
export function JobsLayout({
  ...
  return (
    <div className="min-h-screen bg-white">
      {/* Hero — green-tinted dark gradient, distinct from About (purple), Community (teal), Events (neutral-dark) */}
      <div className="bg-gradient-to-br from-neutral-900 via-teal-950 to-teal-900 py-16 lg:py-20">
        <div className="max-w-7xl mx-auto px-6 lg:px-10">
          <nav aria-label="Breadcrumb" className="mb-6 animate-fade-in">
            <ol className="flex items-center gap-2 text-sm">
              ...
              <li><span className="text-white/50">Opportunities</span></li>
              {title !== "Job Board" && (
                <>
                  <li className="text-white/30">/</li>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 py-16 lg:py-20">
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-16">
          ...
          <aside className="hidden lg:block w-48 shrink-0">
            <nav className="sticky top-24" aria-label="Opportunities pages">
              <ul className="space-y-0">
                {jobsPages.map((page, i) => {
                  const isCurrent = location.pathname === page.path;
                  return (
                    <li key={page.path}>
                      <Link
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
            {prevPage ? (
              <Link to={prevPage.path} className="group flex items-center gap-3 px-5 py-4 rounded-xl hover:bg-white hover:shadow-sm transition-all">
                <svg className="w-5 h-5 text-neutral-400 group-hover:text-purple-500 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path d="M11 17l-5-5m0 0l5-5m-5 5h12" />
                </svg>
                <div>
                  <p className="text-xs text-neutral-400 font-medium">Previous</p>
                  <p className="text-sm font-semibold text-neutral-700 group-hover:text-purple-700 transition-colors">{prevPage.label}</p>
                </div>
              </Link>
            ) : <div />}
            ...
            {nextPage ? (
              <Link to={nextPage.path} className="group flex items-center gap-3 px-5 py-4 rounded-xl hover:bg-white hover:shadow-sm transition-all text-right">
                <div>
                  ...
                  <p className="text-sm font-semibold text-neutral-700 group-hover:text-purple-700 transition-colors">{nextPage.label}</p>
                  <p className="text-xs text-neutral-400 mt-0.5">{nextPage.teaser}</p>
                </div>
                <svg className="w-5 h-5 text-neutral-400 group-hover:text-purple-500 transition-colors shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              <Link to="/sign-up" className="group flex items-center gap-3 px-5 py-4 rounded-xl hover:bg-white hover:shadow-sm transition-all text-right">
                <div>
                  ...
                  <p className="text-xs text-neutral-400 mt-0.5">Free membership, open to all</p>
                </div>
                <svg className="w-5 h-5 text-neutral-400 group-hover:text-purple-500 transition-colors shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path d="M13 7l5 5m0 0l-5 5m5-5H6" />
                </svg>
